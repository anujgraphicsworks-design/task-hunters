...
async function verifyFirebaseToken(token) {
  // 1. Try Local JWT Verification (for tokens generated by /api/auth/*)
  try {
    const decoded = jwt.verify(token, JWT_SECRET);
    if (decoded && (decoded.userId || decoded.email)) {
      return {
        uid: decoded.userId || decoded.email,
        email: decoded.email,
    ...
    // Token is not a local JWT, proceed to external ID token verification
  }

  // 2. Try Firebase Admin Verify
  try {
  ...
  // 3. Try Manual JWT Public Keys Validation (Google x509 certs for Firebase RS256 Tokens)
  try {
    ...
    // Deferred to Google TokenInfo API fallback
  }

  // 4. Try Google OAuth / Firebase ID TokenInfo API (Robust Online Fallback)
  try {
    const googleVerifyRes = await fetch(`https://oauth2.googleapis.com/tokeninfo?id_token=${encodeURIComponent(token)}`);
